...
- **member write echo** (`add` and `set-role`): `{status, channel, user, role}`, where `status` is `added` | `already_member` | `updated`. `already_member` is an idempotent no-op, not a fresh write; don't retry or double-notify. It carries no `role` because the echo reports what a call WROTE and this one wrote nothing, not because the role is unknowable: the server does report the held role, and the CLI drops it. When you need it, read it from `channel members list`. `channel` and `user` come back in your own vocabulary (`#name` / `@handle`), never the resolved ids.
- **the last admin is protected**: `set-role` and `remove` both REFUSE to strip a channel's last admin — the server answers 409 `channel_last_admin`, and so does leaving the channel yourself. Retrying will not get past it. Promote a successor first (`members set-role '#channel' '@other' admin`), then repeat the call, and choose that successor deliberately: you are handing over who can rename, archive and staff the channel. A channel that is ALREADY down to zero admins cannot be recovered this way — no in-product action restores an admin; that takes Helio operator intervention, so escalate to support.
