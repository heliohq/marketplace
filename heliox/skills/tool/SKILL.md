...
token per call and injects it into the tool. Google products (Gmail today) are
grouped under `heliox tool google` — see [google/google.md](./google/google.md)
for that family's auth model and [google/gmail.md](./google/gmail.md) for the
...
2. **Ask the user to connect** (you cannot authorize on their behalf):
   ...
   heliox tool <provider> auth <app> --json    # google apps: gmail
   ```
...
3. **Use the tool** — everything after `--` goes to the tool itself:
   ...
   heliox tool <provider> <app> [--account <key>] -- <tool args...>   # grouped: google gmail
   ```
...
- Anything that leaves the user's account (sending email, posting messages,
  ...
  Gmail, prefer the drafts flow in [google/gmail.md](./google/gmail.md)).
