...
A charter pointer in the channel description looks like
`📋 Team charter: helio://document/<id>`. The pointer is published only after
the human owner ratifies — **a description pointer means a ratified charter** —
so operate under it without re-checking its status.
...
2. Publish the pointer into the channel description — by **merge**, never
   ...
   replace that line, otherwise append `📋 Team charter: helio://document/<id>`
...
(`heliox document edit helio://document/<id> --old ... --new ...`, through
