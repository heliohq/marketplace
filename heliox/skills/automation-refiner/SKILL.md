...
interpret, not just before you edit.
...
Your default is to fix it. An automation decays naturally as the services, channels, and people it depends on change around it. The owner created it so they would not have to watch those things, so repairing that decay is your job and escalating is the fallback, not the first move. What you hold on an automation you execute is wider than it looks, and the server states it rather than leaving you to work it out from who owns what: read `permissions` on any `automation show` or `list --json`, and [`../automation-creator/references/your-authority.md`](../automation-creator/references/your-authority.md) for what each decision opens. `can_edit` is false for you and says nothing about the rest — writing the procedure, arming it, and rehearsing it are all yours.
...
Edit the bound procedure document in place with `heliox document edit <document_id> --old "<exact span>" --new "<replacement>"`. Your binding carries this write, so nothing has to be arranged first: you are editing the steps you are the one who runs. There is no whole-document write: `--old` must match a unique contiguous span. Copy it byte-for-byte from `heliox document read` output, which prints the body as raw markdown, so keep any `##` or `**` markers the span carries. Preserve the automation and trigger identities, because rebuilding them from scratch discards the run history that future cross-run reviews depend on. Recreate only when the change genuinely requires a new automation, meaning a different job or a different owner.
...
Who receives the results is stored on the automation, not in the procedure. Writing "send this to Sarah too" into the steps changes nothing, because every fire snapshots the stored audience. Changing that stored audience is the owner's act, in person: the server refuses subscriber changes from anyone else, including you. `heliox automation subscriber add`/`remove` and `update --subscriber` run under your executor identity and are rejected. Read the effective audience with `heliox automation subscriber list <id>`, which shows every subscriber including the owner, who is always implicit. When the audience needs to change, tell the owner exactly what to change (who to add or remove, on which automation) and let them do it from the automation's page in Helio. Removals matter most here: someone the owner wanted taken off the list keeps receiving the output until the owner removes them, so deliver that message promptly instead of queueing it behind other work.
...
The schedule itself lives on the underlying trigger, not in the procedure document. Change a recurring rule in place: read the current one first with `heliox automation show <id>` (it prints the cron and timezone), then `heliox automation update <id> --cron "<expr>"`, adding `--timezone <IANA>` only when the timezone itself should move; omitted, the current one stays. Convert the owner's phrasing the way the creator skill does: an explicit clock time becomes an exact cron, vague wording gets a randomized minute offset, and you answer in their terms rather than showing the expression. The new rule takes effect at the next fire, and the automation's identity and run history stay intact, so tell the owner what the rule now is before that fire. Two boundaries hold. Only recurring rules can be edited in place: a one-shot's start time and an event trigger's timing cannot, so those changes still go through a replacement: create the new automation, validate it while it is still disabled, disable the old one with `heliox automation update <old-id> --enable false`, then enable the replacement; that order matters, because both live at once fires the same job twice, and disabling beats deleting because the old run history stays readable. And rescheduling is an editor's write, and no executor holds editor, whichever of you created the automation: ask the owner to move it, in conversation or in the app, instead of retrying, and keep `--cron` out of any call that arms the automation, because a PATCH carrying more than `enabled` is refused whole rather than partly applied. Writing "run at 8:30 instead of 9:00" into the procedure does not change when the trigger fires.
...
A rehearsal is a real run and closes like one. Judge it on its ending as well as its output: a rehearsal that produced the right text but finalized without the result reaching the thread has reproduced the defect rather than fixed it.
...
Rehearsing is yours on an automation you execute, so do it while the automation is disabled: `heliox automation run <id> --rehearsal --fire-key <fix>:<n>`, then fetch the transcript through Entry 2's commands to read the result. Disabling stops the schedule from firing on its own; `--rehearsal` keeps a failed attempt out of the consecutive-failure auto-disable count, lets `success` close without subscriber digests, and a retry with the same `--fire-key` reuses the run instead of minting a duplicate. Fire keys are durable per automation, so a reused key returns the prior finished run instead of firing: mint a fresh key for every new fire and reuse one only to retry that same fire. A rehearsal's run card carries no subscriber audience, so subscribers never receive it: leave the subscriber list alone. A fresh executor still wakes up and follows the procedure, so contain any outward side effect the procedure itself performs (mail it sends, external channels it posts to) before you fire.
...
**Then record what the run showed you**, before the terminal verb, if the run
hit something abnormal (an error, an API that refused, a tool you had to work
around) or the result does not hold up against the procedure and the owner's
feedback:
...
- **Tooling limits.** "A one-shot's start time cannot be edited in place" is
  something the *refiner* needs and the executor cannot act on. Experience.
